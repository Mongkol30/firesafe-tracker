...
import {
  Table, Button, Modal, Form, Input, Toggle, SelectPicker,
...
import {
  getExtinguishers, addExtinguisher, updateExtinguisher, deleteExtinguisher,
  type Extinguisher, type ExtinguisherForm
...
const emptyForm: ExtinguisherForm = {
  id: '', extinguisherCode: '', locationId: '', typeId: '',
  lastRefillDate: '', active: true 
}
...
export default function ExtinguisherPage() {
    ...
    try {
      ...
      setLocations(
        locRes.data
          ...
          .map((l: Location) => ({ label: `${l.code} - ${l.name}`, value: l.id }))
      )
      setTypes(
        typeRes.data
          .filter((t: ExtinguisherType) => t.active)
          .map((t: ExtinguisherType) => ({ label: `${t.code} - ${t.name}`, value: t.id }))
      )
    } catch {
  ...
  const handleDownloadQR = () => {
    ...
    icon.setAttribute('y', '44')
    icon.setAttribute('font-size', '28')
    icon.textContent = '🧯'
    card.appendChild(icon)

    // FireSafe Tracker title
    ...
    title.textContent = 'FireSafe Tracker'
    card.appendChild(title)

    const subtitle = document.createElementNS(svgNS, 'text')
    subtitle.setAttribute('x', '62')
    subtitle.setAttribute('y', '54')
    subtitle.setAttribute('fill', 'rgba(255,255,255,0.75)')
    subtitle.setAttribute('font-size', '12')
    subtitle.setAttribute('font-family', 'sans-serif')
    subtitle.textContent = 'Fire Extinguisher Inspection'
    card.appendChild(subtitle)

    // QR code
    const qrClone = qrSvg.cloneNode(true) as SVGElement
    qrClone.setAttribute('x', '50')
    qrClone.setAttribute('y', '84')
    ...
    divider.setAttribute('y1', '296')
    divider.setAttribute('x2', String(cardWidth - 24))
    divider.setAttribute('y2', '296')
          ...
          <Column width={120}>
            ...
            <Cell>
              {(rowData) => {
                ...
                return <Tag color={result === 'PASS' ? 'green' : 'red'}>{result}</Tag>
              }}
            </Cell>
          </Column>
          <Column width={130}>
  <HeaderCell>เติมสารล่าสุด</HeaderCell>
  <Cell>{(rowData) => formatDate((rowData as Extinguisher).lastRefillDate)}</Cell>
</Column>
<Column minWidth={160} flexGrow={1}>
  <HeaderCell>หมายเหตุล่าสุด</HeaderCell>
  <Cell>
    {(rowData) => (
      <span style={{ color: (rowData as Extinguisher).lastRemark ? '#444' : '#aaa', fontSize: 13 }}>
        {(rowData as Extinguisher).lastRemark || '-'}
          ...
          <Column width={120}>
            ...
            <Cell>{(rowData) => formatDate((rowData as Extinguisher).lastInspectedDate)}</Cell>
          </Column>
          <Column minWidth={120} flexGrow={1}>
            ...
            <Cell>
              {(rowData) => (
                <ButtonToolbar>
                  ...
                  <IconButton
                    icon={<span style={{ fontSize: 11, fontWeight: 700 }}>QR</span>}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} size="xs">
        <Modal.Header>
          <Modal.Title>{isEdit ? 'แก้ไขถังดับเพลิง' : 'เพิ่มถังดับเพลิง'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form fluid>
            <Form.Group>
            ...
            <Form.Group>
              ...
              <Input value={form.extinguisherCode} onChange={v => setForm({ ...form, extinguisherCode: v })} />
            </Form.Group>
            <Form.Group>
              <Form.Label>ตำแหน่งที่ตั้ง</Form.Label>
              <SelectPicker data={locations} value={form.locationId}
                onChange={v => setForm({ ...form, locationId: v ?? '' })} cleanable={false} block />
            </Form.Group>
            <Form.Group>
              <Form.Label>ประเภทถัง</Form.Label>
              <SelectPicker data={types} value={form.typeId}
                onChange={v => setForm({ ...form, typeId: v ?? '' })} cleanable={false} block />
            </Form.Group>
            <Form.Group>
  <Form.Label>วันที่เติมสารล่าสุด</Form.Label>
  <Input type="date" value={form.lastRefillDate}
    onChange={v => setForm({ ...form, lastRefillDate: v })} />
            ...
            {isEdit && (
              <>
                <Form.Group>
                  ...
                  <Input value={formatDate(data.find(d => d.id === form.id)?.lastInspectedDate)} disabled />
                </Form.Group>
                <Form.Group>
                  <Form.Label>ชื่อผู้ตรวจล่าสุด</Form.Label>
                  <Input value={data.find(d => d.id === form.id)?.lastInspectorName ?? '-'} disabled />
                </Form.Group>
              </>
            )}
            <Form.Group>
        ...
        <Modal.Footer>
          <Button onClick={handleSave} appearance="primary" loading={saving}
            style={{ background: '#ba0b1b', border: 'none' }}>บันทึก</Button>
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} size="xs">
        <Modal.Header><Modal.Title>ยืนยันการลบ</Modal.Title></Modal.Header>
        <Modal.Body>ต้องการลบถัง <strong>{deleteTarget?.id}</strong> ใช่หรือไม่?</Modal.Body>
        <Modal.Footer>
      ...
      <Modal open={!!qrTarget} onClose={() => setQrTarget(null)} size="xs">
        <Modal.Header>
          <Modal.Title>QR Code — {qrTarget?.extinguisherCode || qrTarget?.id}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {qrTarget && (
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              ...
              <div style={{
                ...
                boxShadow: '0 4px 16px rgba(0,0,0,0.12)', background: 'white',border: '2px solid black' 
              }}>
                {/* Header */}
                <div style={{
                  background: '#ba0b1b', padding: '16px 20px',
                  display: 'flex', alignItems: 'center', gap: 10
                }}>
                  <span style={{ fontSize: 24 }}>🧯</span>
                  <div>
                    <div style={{ color: 'white', fontWeight: 700, fontSize: 13 }}>Fire Extinguisher Inspection</div>
                    {/* <div style={{ color: 'rgba(255,255,255,0.75)', fontSize: 11 }}>Fire Extinguisher Inspection</div> */}
                  </div>
                </div>

                {/* QR */}
                <div id="qr-print" style={{ display: 'flex', justifyContent: 'center', padding: '20px 20px 12px' }}>
                  <QRCodeSVG
                ...
                {/* Info */}
                <div style={{ borderTop: '1px solid #f0f0f0', padding: '12px 20px 20px', textAlign: 'center' }}>
                  <div style={{ fontSize: 11, color: '#888', marginBottom: 4 }}>รหัสถัง</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: '#ba0b1b', marginBottom: 6 }}>
        ...
        <Modal.Footer>
          <Button appearance="primary" onClick={handleDownloadQR}
            style={{ background: '#ba0b1b', border: 'none' }}>
            ดาวน์โหลด SVG
          </Button>
          <Button appearance="subtle" onClick={() => setQrTarget(null)}>ปิด</Button>
        </Modal.Footer>
      </Modal>
    </div>
  )
